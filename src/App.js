...
class App extends React.Component {
			...
			if (getParams.vk_user_id == data.id) {
				try {
					fetch2("startapp").then((data) => {
						let arcades = [];
						data.result.arcades.forEach((el) => {
							arcades.push(
								<ContentCard
									image={el.img}
	...
	setActiveModal(activeModal, profileData) {
		...
		if (
			profileData.battletag !== undefined &&
			profileData.battletag !== null
		) {
			fetch(
			...
			setTimeout(() => {
				this.setState({
					profileDataInfo:
						"Невозможно получить данные об игровом аккаунте.",
	...
	goBack = () => {
		...
		if (
			...
			this.state.activePanel == "textpage"
		)
					...
					subheader={
						<div style={{ textAlign: "left" }}>
							...
							<Div>
								...
								Игровое время: с {this.state.profile.time1}:00
								до {this.state.profile.time2}:00 по МСК <br />
								Микрофон:{" "}
								{this.state.profile.microphone == 1 &&
									"Да"}{" "}
								{this.state.profile.microphone == 0 && "Нет"}{" "}
								<br />
								Discord:{" "}
