...
class FindTeammate extends React.Component {
  ...
  componentDidMount() {
    fetch2("db.GetGameProfile").then((data) => {
      if (data.result !== null) this.setState({ battleTag: data.result });
    });
    setTimeout(() => {
    ...
    fetch2("teammates.getProfile").then((data) => {
      ...
      if (
        data.result[0].description !== null &&
        data.result[0].display == true
      ) {
        fetch2("teammates.getWall").then((data) => {
          let wall = [];
          data.result.map((el) => {
        ...
        if (this.state.profileTag === true) {
          ...
          fetch2("teammates.setSettings", "type=tag&tag=1").then((data) => {
            if (data.result === "ok") {
              ...
              this.props.openTextPage(
                "Включили, проверяй!",
                "Теперь любой человек в ленте увидит твой батлнет-тег.",
                "Понятненько",
                "findteammate",
                true
              );
      ...
      <Panel id={id} className="homePage">
        ...
        >
          <Panel id="feed" activeStory="feed">
            {this.state.spinner ? (
              <PanelHeader separator={false}>Загрузка...</PanelHeader>
            ) : (
              <PanelHeader
                separator={false}
                ...
                <Card style={{ paddingBottom: 5 }}>
                  {this.state.profile[0].verified == 1 && (
                    <SimpleCell
                      ...
                      badge={<Icon12Verified fill="#4cad64" />}
                      before={<Avatar size={40} src={user.photo_200} />}
                    >
                ...
                <Group mode="plain">
                  ...
                  {this.state.profile[0].description !== null && (
                    <SimpleCell
                      ...
                      after={this.state.switches.displayInSearch}
                    >
                      Отображение профиля
                    </SimpleCell>
                  )}
                  ...
                  {this.state.battleTag !== null && (
                    <SimpleCell
                      ...
                      disabled
                      description="Показывать тег профиля?"
                      after={this.state.switches.profileTag}
                    >
                      {this.state.profile[0].battletag
                        ? this.state.profile[0].battletag
                        : "[Скрыт]"}
