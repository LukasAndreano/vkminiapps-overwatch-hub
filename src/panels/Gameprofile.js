...
class Gameprofile extends React.Component {
	...
	componentWillUnmount() {
		this.setState({profileinfo: null, snackbar: null, spinner: false, battletag: null, request: false, allow_messages_request: false});
	}
	connect() {
		if (this.state.request === false && this.state.allow_messages_request === false) {
			...
			{getParams.vk_user_id == this.props.user.id &&
				...
				.catch(error => {
					...
					this.setState({allow_messages_request: true, snackbar:
						<Snackbar
						  onClose={() => this.setState({ snackbar: null })}
						  before={<Avatar src={OverwatchDailyArcadeIcon} size={32} />}
						>
			...
			setTimeout(() => {this.setState({allow_messages_request: false})}, 4000);
		}
	...
	render() {
		...
		return (
		<Panel id={id}>
			<PanelHeader separator={false} left={<PanelHeaderBack onClick={go} data-to="home"/>}>Игровой профиль</PanelHeader>
			...
			<Div>
				<Card>
					<Div>
						...
						<Text weight="regular">В этом резделе отображается вся доступная нам информация по твоему игровому профилю (ТОЛЬКО PC и регион EUROPE). Обратите внимание, что игровой профиль должен быть открыт (это можно сделать в настройках игры). Информация по профилю обновляется в течение 30 минут.</Text>
					</Div>
				</Card>	
				...
				<div>
					...
					<Button style={{marginTop: 10}} onClick={this.connect} size="l" stretched mode="secondary">Привязать профиль</Button>
				</div>
				}
				{this.state.battletag !== null &&
				<div>
					...
					<div>
						...
						<Card>
							<MiniInfoCell textWrap="full" style={{marginTop: 10}}>
								Открыть профиль можно в настройках -> общение -> отображение профиля (поставить: открытый)
							</MiniInfoCell>
						</Card>
					</div>
					}
						...
						<Card style={{marginTop: 10}}>
							<Group>
								...
								<MiniInfoCell before={<Icon24CupOutline />}>
									Соревновательная игра: <br /> Хил - {this.state.profileinfo.competitive.support.rank ? this.state.profileinfo.competitive.support.rank : 'пройди калибровку'} <br /> Урон - {this.state.profileinfo.competitive.damage.rank ? this.state.profileinfo.competitive.damage.rank : 'пройди калибровку'} <br /> Танк - {this.state.profileinfo.competitive.tank.rank ? this.state.profileinfo.competitive.tank.rank : 'пройди калибровку'} 
								</MiniInfoCell>
								<MiniInfoCell before={<Icon24RecentOutline />} textWrap="full">
									Наиграно часов в режиме (формат: ЧАСЫ:МИН:СЕК): <br />Быстрая игра - {this.state.profileinfo.playtime.quickplay ? this.state.profileinfo.playtime.quickplay : '0'} <br />Ранкед (этот сезон) - {this.state.profileinfo.playtime.competitive ? this.state.profileinfo.playtime.competitive : '0'}
								</MiniInfoCell>
							</Group>
						</Card>
						<Card style={{marginTop: 10}}>
						...
						<Card style={{marginTop: 10}}>
							<Group>
								<MiniInfoCell before={<Icon24GameOutline />}>
									Побед в быстрой игре: {this.state.profileinfo.games.quickplay.won ? this.state.profileinfo.games.quickplay.won : '0'}<br />Поражений в быстрой игре: {this.state.profileinfo.games.quickplay.played - this.state.profileinfo.games.quickplay.won ? this.state.profileinfo.games.quickplay.played - this.state.profileinfo.games.quickplay.won : '0'}<br />Матчей всего: {this.state.profileinfo.games.quickplay.played ? this.state.profileinfo.games.quickplay.played : '0'} <br /><br />Побед в ранкеде (этот сезон): {this.state.profileinfo.games.competitive.won ? this.state.profileinfo.games.competitive.won : '0'}<br />Поражений в ранкеде (этот сезон): {this.state.profileinfo.games.competitive.played - this.state.profileinfo.games.competitive.won ? this.state.profileinfo.games.competitive.played - this.state.profileinfo.games.competitive.won : '0'}<br /> Матчей типа 'ничья' (этот сезон): {this.state.profileinfo.games.competitive.draw ? this.state.profileinfo.games.competitive.draw : '0'} <br />Матчей всего (этот сезон): {this.state.profileinfo.games.competitive.played ? this.state.profileinfo.games.competitive.played : '0'}
								</MiniInfoCell>
							</Group>
						</Card>
					</div> 
					...
					<div>
						<Card>
							<MiniInfoCell before={<Icon24ErrorCircleOutline />} textWrap="full" style={{marginTop: 10}}>
					...
					<Button style={{marginTop: 10}} onClick={this.connect} size="l" stretched mode="secondary">Перепривязать профиль</Button>
				</div>
				}
			</Div>
			}
