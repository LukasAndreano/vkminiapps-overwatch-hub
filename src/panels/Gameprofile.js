...
class Gameprofile extends React.Component {
			...
			this.setState({
				snackbar: (
					<Snackbar
						...
						before={
							<Avatar src={OverwatchDailyArcadeIcon} size={32} />
						}
					>
	...
	connect() {
		if (
			...
			this.state.allow_messages_request === false
		) {
									...
									if (data.result === "ok") {
										...
										this.setState({
											...
											snackbar: (
												...
												>
													Мы отправили тебе сообщение
													с ссылкой. Проверь личку,
													пожалуйста.
												</Snackbar>
											),
										});
										...
										this.setState({
											snackbar: (
												...
												>
													Мы уже отправляли тебе
													сообщение. Проверь личку.
												</Snackbar>
											),
										});
									}
					...
					<Div>
						<Card>
							<Div>
								...
								<Text weight="regular">
									В этом резделе отображается вся доступная
									нам информация по твоему игровому профилю
									(ТОЛЬКО PC и регион EUROPE). Обрати
									внимание, что у тебя должен быть создан
									BattleTag и игровой профиль должен быть
									открыт (это можно сделать в настройках
									игры). Информация по профилю обновляется в
									течение 30 минут.
								</Text>
							</Div>
						</Card>
									...
									<div>
										...
										<Card>
											<MiniInfoCell
												...
												style={{ marginTop: 10 }}
											>
												Открыть профиль можно в
												настройках - общение -
												отображение профиля (поставить:
												открытый)
											</MiniInfoCell>
										</Card>
									</div>
								)}
								{this.state.profileinfo.private === false &&
										...
										<div>
											...
											<Card style={{ marginTop: 10 }}>
												<Group>
													...
													>
														Уровень репутации:{" "}
														{this.state.profileinfo.endorsement.frame.substr(
															-1
														)}
														.
														<br /> Тебя хвалили за:{" "}
									...
									<div>
										<Card>
											<MiniInfoCell
												...
												style={{ marginTop: 10 }}
											>
												Мы не получили ответа от сервера
												Blizzard. Попробуйте позже,
												пожалуйста.
											</MiniInfoCell>
										</Card>
									</div>
