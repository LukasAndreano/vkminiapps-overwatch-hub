    ...
    componentDidUpdate() {
        if (this.state.changed === true) {
            if (this.state.subscribed === true) {
                this.setState({
                    changed: false, alert: <SimpleCell disabled onChange={() => {
                        this.props.clickOnLink();
                        this.changeSubscribe(true)
                    }} before={<Icon28Notifications/>} after={<Switch defaultChecked/>}>Рассылка с аркадами</SimpleCell>
                });
            } else {
                this.setState({
                    changed: false, alert: <SimpleCell disabled onChange={() => {
                    ...
                    }} before={<Icon28Notifications/>} after={<Switch/>}>Рассылка с аркадами</SimpleCell>
                });
            }
        }
    }
    ...
    componentDidMount() {
        ...
        setTimeout(() => {
            if (this.props.subscribed === true) {
                this.setState({
                    alert: <SimpleCell disabled onChange={() => {
                        this.props.clickOnLink();
                        this.changeSubscribe(true)
                    }} before={<Icon28Notifications/>} after={<Switch defaultChecked/>}>Рассылка с аркадами</SimpleCell>
                });
            ...
            <Panel id={id} className="homePage">
                <Epic activeStory={this.state.activeStory} tabbar={
                    ...
                    <Panel id="menu" activeStory="menu">
                        <PanelHeader separator={false} left={<PanelHeaderButton onClick={() => go('faq')}><Icon28HelpCircleOutline/></PanelHeaderButton>}>Главная</PanelHeader>
                        <Group>
                            <SimpleCell onClick={() => go('gameprofile')} expandable before={<Icon28Profile/>}>Игровой
                                профиль</SimpleCell>
                            <SimpleCell onClick={() => go('findteammate')} expandable
                                        before={<Icon28SearchLikeOutline/>}>Поиск напарника <span className="new">NEW</span></SimpleCell>
                            <SimpleCell onClick={() => go('update')} expandable before={<Icon28MoonOutline/>}>Лунный
                                Новый год</SimpleCell>
                            <SimpleCell onClick={() => go('mems')} expandable
                                        before={<Icon28MasksOutline/>}>Мемы</SimpleCell>
                            <SimpleCell onClick={() => go('screenshots')} expandable
                                        before={<Icon28FaceRecognitionOutline/>}>Скриншоты персонажей</SimpleCell>
                            <SimpleCell onClick={() => go('arts')} expandable
                                        before={<Icon28PictureStackOutline/>}>Арты</SimpleCell>
                            <SimpleCell onClick={() => go('randomgg')} expandable before={<Icon28SparkleOutline/>}>Случайный
                                голдган</SimpleCell>
                        </Group>
                    </Panel>
                    <Panel id="arcades" activeStory="arcades">
                        ...
                        <Group header={<Header mode="secondary">ИСТОРИЯ АРКАД ЗА ПОСЛЕДНИЕ 3 ДНЯ</Header>}>
                            <CardScroll size="m" style={{marginBottom: "-50px"}}>
                                <ContentCard
                                    ...
                                    onClick={() => {
                                        this.openImage(this.props.arcadesHistory.img1);
                                    }}
                                ...
                                <ContentCard
                                    ...
                                    style={{marginBottom: "50px"}}
                                    onClick={() => {
                                        this.openImage(this.props.arcadesHistory.img2);
                                    }}
                                    header="Аркады вчера"
                                />
                                <ContentCard
                                    ...
                                    style={{marginBottom: "50px"}}
                                    onClick={() => {
                                        this.openImage(this.props.arcadesHistory.img3);
                                    }}
                        ...
                        <Div>
                            <Card>
                                <Div>
                                    <Title level="2" weight="heavy" style={{marginBottom: 10}}>Где, допустим, аркады за
                                        прошлый месяц?</Title>
                                    <Text weight="regular">Здесь отображены аркады за последние 3 дня, включая
                                        сегодняший. Чтобы посмотреть аркады за всё время работы Overwatch Daily Arcade,
                                        нажмите на соответствующую кнопку ниже. Кнопка перенаправит Вас на наше
                                        официальное сообщество.</Text>
                                </Div>
                            </Card>
                        </Div>
                        <Div>
                            <Button size="l" stretched mode="secondary" target="_blank" href="https://vk.com/ow.arcade">Перейти
                                в сообщество</Button>
                        </Div>
                    </Panel>
                </Epic>
