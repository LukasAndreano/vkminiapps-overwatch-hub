    ...
    fetch2("teammates.getProfile").then((data) => {
      if (data.result !== "created") {
        ...
        if (data.result[0].time1 !== null && data.result[0].time2 !== null) {
          this.setState({
            playtime: [data.result[0].time1, data.result[0].time2],
          });
        }
      }
    });
            ...
            this.setState({
              snackbar: (
                <Snackbar
                  ...
                  onClose={() => this.setState({ snackbar: null })}
                >
                  Эй, не так быстро! Обновлять информацию о профиле можно раз в
                  5 сек.
                </Snackbar>
              ),
            });
  ...
  render() {
    ...
    return (
      <Panel id={id}>
        <PanelHeader
          separator={false}
          left={<PanelHeaderBack onClick={() => go("findteammate")} />}
        >
          Настройки
        </PanelHeader>
            ...
            >
              <Div>
                <Card>
                  <Div>
                    <Text weight="regular">
                      Если у тебя закрыты личные сообщения, то открой их,
                      пожалуйста. В ином случае другие пользователи сервиса не
                      смогут тебе написать.
                    </Text>
                  </Div>
                </Card>
              </Div>
              ...
              <FormItem top="Расскажи о себе">
                <Textarea
                  ...
                  onChange={(e) => {
                    this.setState({
                      about: e.target.value.replace(/[@+#+*+?+&++]/gi, ""),
                    });
                  }}
                />
              </FormItem>
              ...
              <FormItem top="Твой Discord-тег?">
                <Input
                  placeholder="Допустим, Lukas#1575"
                  name="discord"
                  maxLength={30}
              ...
              <FormItem top="Твой возраст?">
                <Input
                  ...
                  placeholder="Я ощущаю себя на 39, хотя мне 13!"
                  required
                  maxLength={2}
                  onChange={(e) => {
              ...
              <FormItem>
                <Button size="l" type="primary" stretched mode="secondary">
                  Сохранить
                </Button>
              </FormItem>
            </FormLayout>
