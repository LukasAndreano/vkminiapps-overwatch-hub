	...
	componentDidMount() {
		fetch2("teammates.getProfile").then((data) => {
			if (data.result !== "created") {
				...
				if (
					data.result[0].time1 !== null &&
					data.result[0].time2 !== null
				) {
					this.setState({
					...
					if (data.result === "ok") {
						...
						this.setState({
							snackbar: (
								...
								>
									Эй, не так быстро! Обновлять информацию о
									профиле можно раз в 5 сек.
								</Snackbar>
							),
						});
					} else {
		...
		return (
			<Panel id={id}>
				<PanelHeader
					separator={false}
					left={
						<PanelHeaderBack onClick={() => go("findteammate")} />
					}
				>
					Настройки
				</PanelHeader>
						...
						>
							<Div>
								<Card>
									<Div>
										<Text weight="regular">
											Если у тебя закрыты личные
											сообщения, то открой их, пожалуйста.
											В ином случае другие пользователи
											сервиса не смогут тебе написать.
										</Text>
									</Div>
								</Card>
							</Div>
